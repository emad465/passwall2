...
detect_system_info() {
    ...
    if [ -z "$CPU_MODEL" ]; then
        CPU_MODEL=$(cat /proc/cpuinfo | grep "Processor" | head -n 1 | cut -d: -f2 | xargs)
    fi
    log "INFO" "CPU Model: $CPU_MODEL"
    echo -e "CPU Model: ${GREEN}${CPU_MODEL:-Unknown}${NC}"
    
    # Get memory info (fix KB to MB conversion)
...
show_system_info() {
    ...
    echo -e "${CYAN}OpenWrt Information:${NC}"
    echo -e "  Version: $OPENWRT_VERSION"
    echo -e "  Codename: $OPENWRT_CODENAME"
    echo -e "  Architecture: $OPENWRT_ARCH"
    echo ""
    
    echo -e "${CYAN}Hardware Information:${NC}"
    echo -e "  CPU: $CPU_MODEL"
    echo -e "  System Architecture: $SYSTEM_ARCH"
    echo -e "  Total Memory: ${TOTAL_MEM}MB"
    echo -e "  Free Memory: ${FREE_MEM}MB"
    echo ""
    
    echo -e "${CYAN}Storage Information:${NC}"
    echo -e "  Total Space: $ROOT_TOTAL"
    echo -e "  Used Space: $ROOT_USED ($ROOT_PERCENT)"
    echo -e "  Available Space: $ROOT_AVAIL"
    echo ""
...
install_passwall2() {
    ...
    echo -e "${BOLD}Installation Information:${NC}"
    echo -e "  This will install PassWall2 and its dependencies"
    echo -e "  Required space: ~10-50MB depending on selected packages"
    echo -e "  Available space: ${GREEN}$ROOT_AVAIL${NC}"
    echo ""
    echo -e "${YELLOW}Note: Installation may take several minutes depending on your connection.${NC}"
    echo ""
    ...
    # Get LAN IP address
    local lan_ip=$(uci get network.lan.ipaddr 2>/dev/null || echo "your-router-ip")
...
update_passwall2() {
    ...
    read -p "Do you want to check for updates? (yes/no): " confirm
...
reinstall_passwall2() {
    ...
    if [ -f /tmp/passwall2.backup ]; then
        cp /tmp/passwall2.backup /etc/config/passwall2
...
view_logs() {
    ...
    if [ -f "$LOG_FILE" ]; then
        ...
        echo "No log entries available for current session"
    fi
...
# Trap errors
trap 'log "ERROR" "Script encountered an error at line $LINENO"; exit 1' ERR
